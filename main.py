...
class Hotel:
    def __init__(self, root):
        ...
        self.payment_button = Button(bottom, text="MAKE PAYMENT", font=('', 20), bg="#15d3ba",
                                      relief=RIDGE,
                                      height=2, width=50, fg="black", anchor="center",
                                      command=payment_ui.payment_ui)
        self.payment_button.grid(row=4, column=2, padx=10, pady=10)
        # Add the following button to the Hotel class
        self.room_service_button = Button(bottom, text="ROOM SERVICE", font=('', 20), bg="#15d3ba",
                                      relief=RIDGE,
                                      height=2, width=50, fg="black", anchor="center",
                                      command=room_service_ui.room_service_ui)
        self.room_service_button.grid(row=5, column=2, padx=10, pady=10)


        # button to exit the program
        self.exit_button = Button(bottom, text="EXIT", font=('', 20), bg="#15d3ba", relief=RIDGE, height=1, width=50,
                                  fg="black",
                                  anchor="center", command=quit)
        self.exit_button.grid(row=6, column=2, padx=1, pady=1)
